...
> **Update (post-Phase-2 production fixes)**: Two follow-up changes after deploy. (1) `POST /api/events/discover` no longer runs discovery synchronously inside the HTTP request — a multi-query pass is several real search+LLM round trips and reliably exceeded Render's own proxy timeout (502, not a client-side timeout). It now creates a `job_runs` row, kicks the work off in the background (`triggerJobAsync` in `apps/api/src/jobs/scheduler.ts`), and returns the job id immediately; the dashboard polls `GET /api/jobs/:id`. (2) `WebsiteResearchProvider` (§6) was extended to fetch the event's own page via Tavily's `/extract` endpoint (not just a generic name search) and feed it into the same extraction call — an event's poster/listing page often names exactly who handles its press/photo access (e.g. "Media Partner: X"), which is a far more targeted signal than searching on the venue/org name alone. If that page credits a specific named entity but gives no direct email for them, the extraction reports a `mediaPartnerLead` (name + the exact source URL it was read from — same anti-hallucination rule as contacts), and the provider does one follow-up search+extract for that name before returning. Still never invents an email or bypasses auth/CAPTCHA/paywalls, and a human still picks which discovered contact becomes the opportunity's primary contact.
